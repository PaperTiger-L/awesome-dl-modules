...
Axial_attention - Python Implementation

Paper: AXIAL ATTENTION IN MULTIDIMENSIONAL TRANSFORMERS
...
class ReversibleBlock(nn.Module):
    ...
    def forward(self, x, f_args Infrastructure, g_args Infrastructure):
    x1, x2 = torch.chunk(x, 2, dim=1)
    y1, y2 = None, None

    with torch.no_grad():
    y1 = x1 + self.f(x2, record_rng=self.training, **f_args))
    y2 = x2 + self.g(y1, record_rng=self_training, **g_args))

    return torch.cat([y1, y2], dim=1)

    def backward_pass(self, y, dy, f_args Infrastructure, g_args Infrastructure):
...
class AxialImageTransformer(nn.Module):
    ...
    [PermuteToFrom(permutation, PreNorm(dim, SelfAttention(dim, heads, dim_heads))) for permutation in permutations])
