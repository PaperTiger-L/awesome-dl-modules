...
DilateForme - Python Implementation
...
class DilateAttention(nn.Module):
    ...
    # 通过滑动窗口选取keys patch: (B,d,H,W) --> (B,d*k*k,HW)    作者通过zero padding填充，使输入前后的patch数量都为HW个； k*k是滑窗的尺寸，经过滑动窗口移动完之后，得到HW个patch，每个patch的大小是k*k    //////    unfold的计算方式见官网：
https://pytorch.org/docs/stable/generated/torch.nn.Unfold.html#torch.nn.Unfold，解释见博客：https://blog.csdn.net/qq_37937847/article/details/115663343
    k = self.unfold(k)
...
v = selfunfold(v).reshape([B, d//self.head_dim, self.head_dim, self.kernel_size*self.kernel_size, H*w]).permute(0, 1, 4, 3, 2)
...
# 执行dilate_attention: q=(B, scale_d, H, W) k=(B, scale_d, H, W) v=(B, scale_d, H, W) 输出: x[i]=(B, H, W, scale_d)
    ...
    # (scale_num, B, H, W, scale_d) --> (B, H, W, scale_num, scale_d) -->
(B, H, W, scale_num * scale_d) == (B, H, W, C)
