...
BiFormer - Python Implementation
...
class TopkRouting(nn.Module):
    ...
    with_param: bool, wether incorporate learnable params in routing unit
...
if self.soft_routing: # soft routing, always differentiable (if no detach)
    mul_weight = 'soft'
...
self.kv_down = nn.MaxPool2d(self.kv_downsample_ratio) if
self.kv_downsample_ratio > 1 else nn.Identity()
elif self.kv_downsample_mode == 'avgpool':
    assert self.kv_downsample_ratio is not None
    self.kv_down = nn.AvgPool2d(self.kv_downsample_ratio) if
self.kv_downsample_ratio > 1 else nn.Identity()
    ...
    raise ValueError(f'kv_down_sample_mode {self.kv_downsample_mode} is not surported!')
...
lepe = self.lepe(rearrange(kv[... , self.qk_dim:], 'n (j i) h w c -> n c
...
topk*h_kv*w_kv, c_kq//m) transpose here?
...
# (bs, nhead, q_nregion, reg_size, head_dim) @ (bs, nhead, q_nregion,
head_dim, topk*kv_region_size)
# -> (bs, nhead, q_nregion, reg_size, topk*kv_region_size)
# TODO: mask padding region
attn = (query * scale) @ key_g.flatten(-3, -2).transpose(-1, -2)
attn = torch.softmax(attn, dim=-1)
# (bs, nhead, q_nregion, reg_size, topk*kv_region_size) @ (bs, nhead,
q_nregion, topk*kv_region_size, head_dim)
...
class BiLevelRoutingAttention_nchw(nn.Module):
    ...
    * uses nchw input format to avoid frequent permutation
    ...
    when the size of inputs is not divisible by the region size, there is also a
numerical difference
    than legacy implementation, due to:
    * different way to pad the input feature map (padding after linear
projection)
    * different pooling behavior (count_include_pad=False)

    Current implementation is more reasonable, hence we do not keep backward
...
side_dwconv=3, auto_pad=False,
    ...
    self.scale = qk_scale or self.dim ** -0.5  # NOTE: to be consistent with
old models.
