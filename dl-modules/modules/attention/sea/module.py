...
Sea - Python Implementation

Paper: SeaFormer: Squeeze-Enhanced Axial Transformer for Mobile Semantic Segmentation
...
qrow = self.pos_emb_rowq(q.mean(-1)).reshape(B, self.num_heads, -1, H).permute(0, 1, 3, 2) #通过平均池化压缩水平方向,并为垂直方向的空间位置添加位置嵌入:(B,C_qk,H,W)--->mean-->(B,C_qk,H)--->reshape-->(B,h,d,H)--->permute-->(B,h,H,d);
...
## squeeze column
qcolumn = self.pos_emb_columnq(q.mean(-2)).reshape(B, self.num_heads, -1, W).permute(0, 1, 3, 2) # 通过平均池化压缩垂直方向,并为水平方向的空间位置添加位置嵌入:(B,C_qk,H,W)--->mean-->(B,C_qk,W)--->reshape-->(B,h,d,W)--->permute-->(B,h,w,d);
C_qk=h*d, h:注意力头的个数; d:每个注意力头的通道数
...
xx = v.add(xx) # 添加残差连接
xx = self.proj(xx) # 应用1x1Conv得到Squeeze Axial attention的输出
